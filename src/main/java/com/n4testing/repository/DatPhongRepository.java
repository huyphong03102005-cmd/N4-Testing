...
public interface DatPhongRepository extends JpaRepository<DatPhong, String> {
    ...
    List<DatPhong> findByTrangThaiIn(List<String> trangThais);
    ...
    @Query("SELECT d FROM DatPhong d LEFT JOIN FETCH d.chiTietDatPhongs ct LEFT JOIN FETCH ct.phong " +
           ...
           "OR d.sdtNguoiDat LIKE CONCAT('%', :keyword, '%')) AND d.trangThai IN :statuses")
    List<DatPhong> searchBookings(@Param("keyword") String keyword, @Param("statuses") List<String> statuses);
